...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'buck-legacy', 'BuckLegacy', and 'BUCKLEGACY'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.buck-legacy/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.buck-legacy/g),
   ...
   * Rename files.
   *
   * Rename files that had buck-legacy in their name, such as
   * css/buck-legacy.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*buck-legacy*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('buck-legacy', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-buck-legacy-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob BuckLegacy's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace buck-legacy name mentions in files.
      generator.replaceFileContents();
      // Rename files that had buck-legacy in their name.
